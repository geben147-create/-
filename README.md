...
보컬 체인 검증: 일부러 30 cents 어긋나게 만든 테이크 → 보정 후 **0.8 cents** (6음 감지, 5음 보정).
사람이 지정한 노트 수정 5건 전부 오디오에 반영되었고, 증빙에는 반영된 개수만 기록됩니다.
...
30개 단위 테스트가 통과합니다. 리미터 릴리스 수식이 기준 재귀식과 1e-9 dB 이내로 일치하는지,
