...
| 항목 | 모델 | 건수 | 크레딧 |
|---|---|---|---|
| 첫 프레임 이미지 | `google / nano-banana-pro` (2K) | 7 | **228** |
| 배경 영상 | `kling-ai / kling-v2-5-turbo` (pro, 5s) | 7 | **210** (30 × 7) |
| 웹 인코딩 · 조립 · 검수 | ffmpeg · 순수 HTML/CSS/JS · Playwright | — | **0** |
| | | **합계** | **438** |

잔액 **12,040 → 11,602**. 기존 연간 구독(2027-08 만료)의 잔여 크레딧에서만 차감됐고
**추가 결제는 없다.**

> 크레딧은 견적이 아니라 **잔액 차이로 실측**했다. Pollo 견적 API 는 이미지에 대해
> `discountCost: 0` 을 돌려주지만 실제로는 차감된다 — 견적값을 믿으면 안 된다.
...
배경 영상 자리에 **순백 영상**을 넣고 돌린다. 스크림이 순백을 견디면 어떤 프레임에서도
글자가 읽힌다 — 가이드가 말하는 &ldquo;평균이 아니라 최악 프레임 기준&rdquo;을 그대로 구현한 것.

검사 항목: 7개 뷰포트 가로 오버플로 · 히어로 카피 잘림 · 콘솔 에러 · 영상 위 텍스트 15곳의
대비비 · `elementFromPoint` 로 확인하는 스택 컨텍스트 · 조상의 `transform`/`filter`/`opacity` ·
스크럽 핀 고정 · 동작 줄이기.

현재 상태: **전체 통과** (`verify/out/report.json`)
