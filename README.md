...
| 항목 | 모델 | 건수 | 크레딧 |
|---|---|---|---|
| 첫 프레임 이미지 | `google / nano-banana-pro` (2K) | 7 | 약 38 × 7 ≈ **266** |
| 배경 영상 | `kling-ai / kling-v2-5-turbo` (pro, 5s) | 7 | 30 × 7 = **210** |
| 웹 인코딩 · 조립 · 검수 | ffmpeg · 순수 HTML/CSS/JS · Playwright | — | **0** |

> 크레딧은 **잔액 차이로 실측**한 값이다. Pollo 의 견적 API 는 이미지에 대해
> `discountCost: 0` 을 돌려주지만 실제로는 건당 약 38 크레딧이 차감된다 —
> 견적값이 아니라 잔액을 믿을 것.

기존 연간 구독(2027-08 만료)의 잔여 크레딧에서만 차감됐고 **추가 결제는 없다.**
